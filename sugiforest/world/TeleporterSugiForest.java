...
public class TeleporterSugiForest extends Teleporter
{
	...
	public boolean placeInExistingPortal(Entity entity, float rotationYaw)
	{
			...
			for (int l = -128; l <= 128; ++l)
			{
				...
				for (int i1 = -128; i1 <= 128; ++i1)
				{
					for (BlockPos blockpos = blockpos4.add(l, worldObj.getActualHeight() - 1 - blockpos4.getY(), i1); blockpos.getY() >= 0; blockpos = blockpos1)
					{
						...
						if (worldObj.getBlockState(blockpos).getBlock() == SugiBlocks.sugi_portal)
						{
							while (worldObj.getBlockState(blockpos1 = blockpos.down()).getBlock() == SugiBlocks.sugi_portal)
							{
								blockpos = blockpos1;
							}
		...
		if (d0 >= 0.0D)
		{
			...
			if (worldObj.getBlockState(((BlockPos)object).west()).getBlock() == SugiBlocks.sugi_portal)
			{
				facing = EnumFacing.NORTH;
			}

			if (worldObj.getBlockState(((BlockPos)object).east()).getBlock() == SugiBlocks.sugi_portal)
			{
				facing = EnumFacing.SOUTH;
			}

			if (worldObj.getBlockState(((BlockPos)object).north()).getBlock() == SugiBlocks.sugi_portal)
			{
				facing = EnumFacing.EAST;
			}

			if (worldObj.getBlockState(((BlockPos)object).south()).getBlock() == SugiBlocks.sugi_portal)
			{
			...
			if (facing != null)
			{
				...
				BlockPos blockpos2 = ((BlockPos)object).offset(facing);
				boolean flag2 = func_180265_a(blockpos2);
				boolean flag3 = func_180265_a(blockpos2.offset(facing2));

				if (flag3 && flag2)
				{
					object = ((BlockPos)object).offset(facing2);
					facing = facing.getOpposite();
					facing2 = facing2.getOpposite();
					BlockPos blockpos3 = ((BlockPos)object).offset(facing);
					flag2 = func_180265_a(blockpos3);
					flag3 = func_180265_a(blockpos3.offset(facing2));
				}
				...
				d4 += facing2.getFrontOffsetX() * f6 + facing.getFrontOffsetX() * f1;
				d6 += facing2.getFrontOffsetZ() * f6 + facing.getFrontOffsetZ() * f1;
				float f2 = 0.0F;
				float f3 = 0.0F;
				float f4 = 0.0F;
				float f5 = 0.0F;
				...
				else if (facing == facing1.getOpposite())
				{
					f2 = -1.0F;
					f3 = -1.0F;
				}
				else if (facing == facing1.rotateY())
				{
					f4 = 1.0F;
					f5 = -1.0F;
				}
				...
				entity.rotationYaw = rotationYaw - facing1.getHorizontalIndex() * 90 + facing.getHorizontalIndex() * 90;
			}
			else
			{
			...
			for (i3 = -1; i3 <= 1; ++i3)
			{
				for (j3 = 1; j3 < 3; ++j3)
				{
					for (k3 = -1; k3 < 3; ++k3)
					{
						...
						worldObj.setBlockState(new BlockPos(l3, i4, j4), flag ? SugiBlocks.sugi_log.getDefaultState() : Blocks.air.getDefaultState());
					}
				}
			}
		}

		IBlockState blockstate = SugiBlocks.sugi_portal.getDefaultState().withProperty(BlockPortal.AXIS, l5 != 0 ? EnumFacing.Axis.X : EnumFacing.Axis.Z);
		...
		for (j3 = 0; j3 < 4; ++j3)
		{
			for (k3 = 0; k3 < 4; ++k3)
			{
				for (l3 = -1; l3 < 4; ++l3)
				{
					...
					worldObj.setBlockState(new BlockPos(i4, j4, k4), flag1 ? SugiBlocks.sugi_log.getDefaultState() : blockstate, 2);
				}
			}
