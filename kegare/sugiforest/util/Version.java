package kegare.sugiforest.util;

import com.google.common.base.Optional;
import com.google.common.base.Strings;
import com.google.common.primitives.Ints;
import kegare.sugiforest.core.SugiForest;
...
public class Version implements Runnable
{
	...
	public void run()
	{
		try
		{
			CURRENT = Optional.of(SugiForest.metadata.version);
			...
			for (int count = 0; count < 3 && !LATEST.isPresent(); ++count)
			{
				if (count == 0)
				{
					SugiLog.info("Beginning version check.");
				}
				...
				latest = properties.getProperty("sugiforest.latest");

				if (Strings.isNullOrEmpty(latest))
				{
					SugiLog.warning("Version check attempt " + count + " failed, trying again in 10 seconds..");
